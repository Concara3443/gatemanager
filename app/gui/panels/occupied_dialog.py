...
def open(app):
    ...
    if app._occ_win and app._occ_win.winfo_exists():
        refresh(app)
        app._occ_win.lift()
        return
    win = tk.Toplevel(app)
    ...
    tree = ttk.Treeview(
        win, columns=cols, show="headings", style="Occ.Treeview", selectmode="browse"
    )
...
def refresh(app):
    ...
    if not app._occ_win or not app._occ_win.winfo_exists():
        return
    for r in app._occ_tree.get_children():
        app._occ_tree.delete(r)
    stands = sorted(app.occupied, key=lambda x: (pf.get_numeric_id(x), x))
    for s in stands:
        i = app.occupied_by.get(s, {})
        cs, ac, air = i.get("cs", "—"), i.get("acft", "—"), i.get("airline", "—")
