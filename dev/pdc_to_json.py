    ...
    for part in parts:
        ...
        if "-" in part:
            ...
            for i, k in enumerate(sorted_keys):
                ...
                if k_up == start_val or re.sub(r"\D", "", k_up).lstrip("0") == start_val.lstrip(
                    "0"
                ):
                    if start_idx == -1:
                        start_idx = i
    ...
    while True:
        ...
        if opc == "1":
            ...
            while True:
                ...
                if not sub_opc or sub_opc not in opts:
                    break
                val = opts[sub_opc]
                rng = input(f"Stands para {val}: ").strip()
                matched = parse_range(rng, keys)
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    for i, h in enumerate(header):
                        ...
                        if "PRKG" in h_upper:
                            idx_prkg = i
                        elif "MAX ACFT" in h_upper:
                            idx_max_acft = i
                        elif "REMARKS" in h_upper or "OBSERVACIONES" in h_upper:
                            idx_remarks = i

                    if idx_prkg != -1 and idx_max_acft != -1:
                        last_table_was_parking, last_indices = (
                            True,
                            (idx_prkg, idx_max_acft, idx_remarks),
                        )
                        rows = table[1:]
                    ...
                    for row in rows:
                        if not row or len(row) <= max(idx_prkg, idx_max_acft):
                            continue
                        pid = str(row[idx_prkg]).strip() if row[idx_prkg] else ""
                        if not pid or "PRKG" in pid.upper() or "PARKING" in pid.upper():
                            continue
                        max_raw = (
                            str(row[idx_max_acft]).replace("\n", " ").strip()
                            if row[idx_max_acft]
                            else ""
                        )
                        rem_raw = ""
