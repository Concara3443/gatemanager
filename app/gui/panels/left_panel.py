...
def build(app, parent):
    ...
    app.v_callsign, app.v_airline, app.v_aircraft, app.v_origin = (
        tk.StringVar(),
        tk.StringVar(),
        tk.StringVar(),
        tk.StringVar(),
    ...
    app._ga_btn = tk.Checkbutton(
        ...
        text="GA",
        font=FONT_S,
        variable=app._ga_var,
        bg=C["bg2"],
        fg=C["fg_dim"],
        selectcolor=C["seg_on"],
    ...
    app._country_lbl = tk.Label(cs_row, text="", font=("Consolas", 8), bg=C["bg2"], fg=C["fg_dim"])
    app._country_lbl.pack(side=tk.LEFT, padx=(3, 0))

    fields = [
        ("Airline", app.v_airline, "optional"),
        ("Aircraft", app.v_aircraft, "optional"),
        ("Origin", app.v_origin, "optional"),
    ]
    for lbl, var, hint in fields:
        row = tk.Frame(inp, bg=C["bg2"])
        row.pack(fill=tk.X, pady=2)
        tk.Label(
            row, text=f"{lbl:<9}", font=FONT_S, bg=C["bg2"], fg=C["fg_dim"], width=9, anchor="w"
        ).pack(side=tk.LEFT)
        tk.Entry(
            row,
            textvariable=var,
        ...
        if hint:
            tk.Label(row, text=hint, font=("Consolas", 8), bg=C["bg2"], fg=C["fg_dim"]).pack(
                side=tk.LEFT, padx=3
            )
    ...
    for l, opts in filter_rows:
        ...
        row.pack(fill=tk.X, pady=2)
        tk.Label(
            row, text=f"{l:<9}", font=FONT_S, bg=C["bg2"], fg=C["fg_dim"], width=9, anchor="w"
        ).pack(side=tk.LEFT)
        sf = tk.Frame(row, bg=C["bg2"])
        sf.pack(side=tk.LEFT)
        app.seg[l] = SegGroup(sf, opts, opts[0][0])

    # buttons
    brow = tk.Frame(inp, bg=C["bg2"])
    brow.pack(fill=tk.X, pady=(6, 2))
    _btn(brow, "Query  ▶", app._query_manual, bg="#0d47a1").pack(
        side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 3)
    )
    _btn(brow, "Aurora  F5", app._query_aurora, bg="#1b5e20").pack(
        side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 3)
    )
    ...
    obs = tk.Frame(of, bg=C["bg"])
    obs.pack(fill=tk.X, pady=(3, 0))
    _btn(obs, "⟳  Sync Aurora", app._sync_occupied_aurora, bg="#0a2a1a").pack(
        side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2)
    )
    _btn(obs, "▦  View all", app._open_occupied_panel, bg="#1a2a3a").pack(
...
def _strip_update(app, cs, air, acft, dep, st, sch, term):
    ...
    r1.pack(fill=tk.X)
    tk.Label(r1, text=cs or "—", font=FONT_L, bg=C["strip_bg"], fg="#000", padx=6, pady=3).pack(
        side=tk.LEFT
    )
    tk.Label(r1, text=acft or "", font=("Consolas", 11), bg=C["strip_bg"], fg="#333").pack(
        side=tk.LEFT, padx=6
    )
    _term_colors = ["#0d47a1", "#880e4f", "#1b5e20", "#e65100", "#4a148c"]
    tbg = (
        _term_colors[app.terminals.index(term) % len(_term_colors)]
        if term in app.terminals
        else "#4e342e"
    )
    tk.Label(r1, text=f" {term} ", font=("Consolas", 9, "bold"), bg=tbg, fg="#fff").pack(
        side=tk.RIGHT, padx=5, pady=3
    )
    tk.Frame(f, bg=C["strip_sep"], height=1).pack(fill=tk.X)
    r2 = tk.Frame(f, bg=C["strip_bg"])
    r2.pack(fill=tk.X)
    tk.Label(r2, text=air or "", font=FONT_S, bg=C["strip_bg"], fg="#444", padx=6, pady=2).pack(
        side=tk.LEFT
    )
    if dep:
        tk.Label(r2, text=dep, font=("Consolas", 9, "bold"), bg=C["strip_bg"], fg="#000").pack(
            side=tk.LEFT, padx=4
        )
    if sch:
        sbg = "#1b5e20" if "NON" not in sch else "#b71c1c"
        tk.Label(r2, text=f"  {sch}  ", font=("Consolas", 8), bg=sbg, fg="#fff", pady=2).pack(
            side=tk.LEFT, padx=4
        )
    tk.Frame(f, bg=C["strip_sep"], height=1).pack(fill=tk.X)
    ...
    if st:
        tk.Label(
            r3, text=f"  STAND  {st}  ", font=("Consolas", 16, "bold"), bg="#111", fg="#fff", pady=5
        ).pack(side=tk.LEFT)
        tk.Label(r3, text="ASSIGNED", font=("Consolas", 8), bg="#111", fg=C["green"]).pack(
            side=tk.LEFT, padx=4
        )
    else:
        tk.Label(
            r3, text="  STAND  —  PENDING", font=("Consolas", 12), bg="#111", fg="#555", pady=5
        ).pack()
